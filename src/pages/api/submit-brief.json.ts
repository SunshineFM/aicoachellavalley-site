...
async function createGithubIssue(input: {
  ...
  const issueBody = [
    "## Brief Submission",
    "",
    `- Submitted at: ${input.submission.submittedAt}`,
    `- Source URL: ${input.submission.sourceUrl}`,
    `- City: ${input.submission.city || "(not provided)"}`,
    `- Sector: ${input.submission.sector || "(not provided)"}`,
    `- Date: ${input.submission.date || "(not provided)"}`,
    `- Submitter Name: ${input.submission.submitterName || "(not provided)"}`,
    `- Submitter Email: ${input.submission.submitterEmail || "(not provided)"}`,
    ...
    "",
    "### Notes",
    input.submission.notes || "(not provided)",
  ].join("\n");
